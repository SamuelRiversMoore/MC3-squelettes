...
function formulaires_edition_doc_charger_dist( $id_document = 'new' , $lien_objet = null, $lien_id_objet = null, $redirection = null, $mode = null){
    ...
    if ( $id_document == 'new' ){
      $retour = array( 'id_document' => 'new', 'nouveau' => 'oui', mode=> $mode ); // Renvoi un objet vide
    }
    else {
...
function formulaires_edition_doc_verifier_dist( $id_document = 'new' , $lien_objet = null, $lien_id_objet = null, $redirection = null, $mode = null){
    ...
    if (!$_FILES['fichier_upload']['tmp_name'] && !$fichier_distant && !$ancien_fichier) $erreur['fichier_manquant'] = 'vous devez indiquer un fichier'; // si aucun fichier renseigné alors erreur
